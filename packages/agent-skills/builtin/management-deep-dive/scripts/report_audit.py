#!/usr/bin/env python3
"""Report Audit Tool — Research Canvas financial-data skill (stdlib only).

数据抽检工具：从研究报告中抽取 15% 的财务数据点，与可靠信源比对，
通过则准出，不通过则打回并说明原因。

主路径取数：平台工具（财务三表 / 公告 / 机构研报 / 用户 panels），
禁止把外网爬虫当作主路径。
...
工作流程（三步）：
  ...
  Step 2 — Agent 对抽检清单中的每个数据点，用 Research Canvas 财务/公告工具或第二源 panels 取数，
            填入 fetched_value
...
def render_verdict(results: list, report_name: str = "", *, quiet=False) -> dict:
    """
    根据核验结果输出准出/打回判决。
    ...
    if fail_count == 0:
        ...
        if not quiet:
            ...
            for fi in fail_items:
                ...
                print()
        verdict = 'FAIL'
        summary = f'{fail_count} 个数据点核验不通过，报告需修正后重审。'

    if warn_count > 0 and not quiet:
...
工作流程：
  ...
  Step 2 — Agent 对清单中每个数据点，用 Research Canvas 财务工具或第二源 panels 取数，
            填入 fetched_value / fetched_source / fetched_value2 / fetched_source2

  Step 3 — 输入核验结果，输出准出/打回判决：
    python3 scripts/report_audit.py verdict --results '[
      {"id":1,"label":"营业收入","reported_value":7518,"unit":"亿","fetched_value":7518,"fetched_source":"opptrix_financials","fetched_value2":7500,"fetched_source2":"notices"},
      ...
    ]'
  ...
  固定随机种子（复现同一批样本）：
    ...
    if args.command == 'extract':
        ...
        print('  主路径：Research Canvas get_instrument_financials / 三表 / financial_indicators / snapshot')
        print('  第二源：get_instrument_institution_report 或用户 panels 双源（公告数据源已下线）')
        print('  补充（非主路径）：公开年报 PDF / 交易所披露；勿把外网爬虫当主路径')
        print()
