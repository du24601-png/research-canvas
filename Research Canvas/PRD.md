...
产品 Demo 验收与 **Agent 决策质量评测** 分开：前者证明主路径能出可溯源图；后者衡量模型会不会选错工具、走错决策链。评测不替代本节六条 Demo。

---

## 18. Agent 质量评测（与产品验收分离）

目标：每次改提示词、工具描述或决策夹具后，能回答「变好了还是变差了、坏在哪一层」，而不是给一个加权总分。

### 18.1 原则

- **LLM 决定、代码执行**：评测只判工具选择与决策链；数字仍须来自 Provider，评测夹具不打真行情。
- **Preview → Adopt**：`propose_widget` 只提议；用户未 Adopt 不得把预览当右侧已有图去 `update_widget`。
- **三层不同构念**：第一轮工具 ≠ 多轮决策链 ≠ 生产 system prompt。禁止合成「Agent 分」，禁止并入产品 CI 门禁。
- **产品北星**是用户真实打到的系统提示词；规划夹具满分只说明量具下主路径能走完。

### 18.2 覆盖的业务场景

与第 2 节用户故事对齐，按场景看失败，而不是只报总准确率：

| 场景 | 期望 |
|---|---|
| 具名公司对比财务/走势 | `query_data` → `propose_widget` |
| 超过 3 家或行业对比 | 先计划/确认样本，禁止把候选当全行业 |
| 只问现价 | 报完即停，不顺手 K 线或快照 |
| 改未 Adopt 的预览 | `propose_widget` / `update_proposal` |
| 改右侧已有组件 | `update_widget` |
| 指数成分股 | 无对应工具则停，禁止编名单 |
| 不该取数的问题 | 零工具 |

### 18.3 怎么跑、怎么读

命令、指标口径、正式分数与读分矩阵见公开文档 [docs/AGENT-EVAL.md](../docs/AGENT-EVAL.md)。改 Agent 后跑 `npm run eval:agent`，对照上一份正式档的有效占比、失败频率、决策次数、约束击穿与新失败题再改。

真数据正确性、用户真 Adopt 进画布是下一层，与本节正交。
