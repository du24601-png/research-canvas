...
日常研究路径：`query_data` 取数 → Agent 提议图表（Preview）→ 你确认 **Adopt** 后写入右侧画布。画布支持拖拽、缩放、整理布局；Preview 与 Widget 可在折线 / 排名 / 热力 / 表格间切换；图表随窗口大小自动收放图例与轴标签（小窗靠悬停看详情）。图种含 K 线、堆积柱、饼图等。
...
| **Research Canvas** | 网格布局 + ECharts；数据集可追溯、可 refine；Preview 未 Adopt 前可改样式 |
| **报告与脑图（可选）** | 输入框 + 菜单勾选后启用网页报告 / TSX 画布 / 脑图；与右侧研究画布分离 |
| **多市场数据** | A 股 / 美股 / 港股等；经统一 Engine 与 Provider 接入 |
...
> 需要自备大模型 API Key（OpenAI 兼容或本地推理）。
...
浏览器打开 `https://127.0.0.1:5173`（自签名证书；可设 `WEB_HTTPS=0` 使用 HTTP）。
